...
| **项目改动面板** | 右上角抽屉，跟随当前对话所属项目；宽度可拖动（双击复位、方向键微调），列表按「状态徽标 + 目录压暗 + 文件名」排布（参照 IDEA 的 Git 工具窗），展开即看逐行差异与提交历史 |
...
- **宽度可拖动**（左边缘手柄；双击复位，聚焦后用 ←/→ 微调、Home 复位），按应用记忆
- 列表参照 IDEA 的 Git 工具窗：状态徽标（A/M/D/R）+ 目录压暗、文件名留亮的路径 + 右对齐的增删行数，展开后是带行号与增删底色的差异，下面是最近提交（含短哈希与作者/日期）
