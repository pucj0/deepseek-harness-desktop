...
| **项目改动面板** | 右上角抽屉，跟随当前对话所属项目；顶部 `Changes / Log` 两个页签（`Changes`：已暂存 / 更改 / 未跟踪三组 + 固定底部的提交区，点文件看逐行差异；`Log`：分支树 + 提交图 + 详情三栏，分栏宽度可拖动并记忆），宽度可拖动（双击复位、方向键微调）。工作区自己不是 Git 仓库时（仓库在子目录里）会自动发现子仓库并列出仓库选择器 |
| **分支徽章与切换** | 输入框上方显示当前分支与本轮修改入口，长分支名自动省略；分支列表按 IDEA 的习惯：**单击选中并打开操作菜单、双击才切换**，右键同一套菜单。多仓库项目在最前面显示 `Git · N 个仓库` 并给出仓库选择器 |
...
- **多仓库项目**（工作区自己不是仓库、仓库在子目录里）在最前面显示 `Git · 2 个仓库` 并给出仓库选择器：分支与改动数**永远只是当前仓库的**，换仓库会立刻重取 status / 分支列表 / 远端列表。单仓库时这一整块**不渲染**，请求里也不多带一个参数
- 点击展开分支切换菜单，菜单顶部是**搜索框**：打开即聚焦并清空，输入即时过滤
- 分支行按 IDEA 的习惯交互：**单击**选中该行并打开它的操作菜单（约 200ms 的延迟用于区分双击），**双击**才切换分支，**右键**打开的是同一个菜单
- 操作菜单是**外侧级联**（IDEA 的分支弹窗同款）：二级菜单优先出现在一级面板**右侧**（`面板右边缘 + 6px`），右侧空间不足时出现在左侧，两边都放不下才在视口内兜底；纵向按被点的那一行对齐并自动上翻。它**不会盖住一级面板**，也不会在面板关掉之后留在屏幕上——关面板统一走"面板 + 二级菜单 + 对话框 + 待弹定时器一起收"这一条路径，渲染层再加一道 `open === true` 的闸门
...
- **作用域分两层：`workspaceRoot`（打开的目录）与 `repositoryRoot`（Git 工作树顶层）。** 仓库根由 host 用 `git rev-parse --show-toplevel` 推导（有界缓存 + single-flight，**不接受客户端传入**，否则 `repositoryRoot=C:/` 就能越过安全边界），所有 Git 命令都以它为 cwd，路径一律**仓库相对**。因此工作区是仓库子目录时（例如 `…/pages/mse`），`Changes` 照样列出**整个仓库**的改动（`root.txt` / `src/a.js` / `pages/x.json`），也不会出现 `../src/a.js` 这种基准漂移。
- **工作区自己不是仓库时不再误报"不是 git 仓库"。** 项目级作用域由 `/project-git-scope` 一条路由回答"这个项目里有哪些仓库"：先探工作区自身（`.git` 允许是文件——worktree / submodule 都是这种形状，因此判存在后再跑一次 `rev-parse --show-toplevel --absolute-git-dir` 确认），再看子目录。发现是**有界**的 BFS：深度上限 4、访问目录上限 4000、快路径（自身 + 第一层）250 ms 预算（超预算就先把已找到的答案交出去，后台继续）、并发 8，并且**不下钻** `node_modules` / `dist` / `build` / `target` / `.next` / `vendor` / `__pycache__` 等 14 个依赖与产物目录（那里的 `.git` 是幽灵仓库）；发现一个仓库后**继续扫兄弟目录**，所以 `haiweiNew/haiwei-manage-fronted` 与 `…/haiwei-manage-backend` 会同时列出来。结果按工作区缓存 60 秒 + single-flight，**不进 10 秒轮询**。`isRepo:false` 只在这份清单真的是空的时候才给——界面因此说"项目 X 里没有发现 Git 仓库"，而不是把责任推给工作区路径。
- **多仓库项目里"当前在看哪个仓库"是整个面板的状态。** 选择器住在抽屉顶部标题栏里、`Changes / Log` **上方**（面板的第一视觉区域，切页签时它的位置与 DOM 都不动）：多仓库时是 `[仓库名 ▾]`，菜单每一项写清**仓库名 / 分支 / 改动数**（当前项带 ✓，同名仓库靠悬停的完整路径区分），菜单最后一行是"后台还在发现更多仓库"的轻量状态；单仓库时**不渲染下拉**，只静态显示 `仓库名 · 分支`。`Changes` 的文件列表与差异、`Log` 的分支树 / 提交图 / 详情、底部提交框**整体**跟着它走；换仓库时两个页签的内容子树一并换新（`Log` 重新拉新仓库的第一页提交图，提交框里针对旧仓库的草稿与 AI 建议也一起清掉），每个仓库仍然各占一格快照、各一套轮询（同一仓库的两个子目录照旧共用一份）。用户没选过时用确定性的默认项（工作区自己所属的仓库 → 列表里的第一个），宿主与客户端用**同一条**规则，因此不会出现"选择器写着 A、面板里是 B"。徽标上的数字是所有仓库**之和**，并缀一句"几个仓库"；**暂存与提交永远只作用于选中的那一个仓库**（客户端为多仓库请求带上 `repository`，宿主只接受"本项目发现过的仓库"，越界一律 400 `repositoryNotAllowed`）。单仓库项目的请求**一个多余参数都不带**，与 1.5.2 完全一致。
- **同一个仓库只共享一份快照、一套轮询。** 快照 store 的键是**仓库根**，所以 `repo/src` 与 `repo/pages` 落在同一条记录上：切一个子目录不清空分支/Changes/徽标、不重新全量扫描、也不会多起一套 10 秒轮询。
- **常驻轮询走"快路径"，Git 工作量与未跟踪文件数量脱钩。** `/workspace` 与 `/status` 用 `status --porcelain=v2 --branch -z --untracked-files=normal`：整块未跟踪目录被折叠成一条 `tmp/`，因此 6,846 个未跟踪文件在轮询里只是 1~2 条记录（实测同一夹具：`-uall` **174.4 KB / 6,855 条**→`-unormal` **0.6 KB / 10 条**；`/workspace` **96 ms / 1.6 KB**，**不向渲染进程传任何未跟踪路径**）。`changedFiles` 精确时带 `changedFilesExact: true`，折叠状态下明确标成估计值。
...
- 顶部是 `Changes / Log` 两个页签（IDEA 的 Git 工具窗同款）：
  - `Changes`：**已暂存 / 更改 / 未跟踪** 三组（同一份快照过滤得出，分组数量与列表恒等），分成**左右两栏**——左栏勾选"我要提交哪些文件"（每行可暂存 / 取消暂存 / 还原 / 查看该文件的变更记录），右栏是"这个文件具体改了什么"的差异预览；两栏之间可拖动（默认左栏占 **34%**，拖动范围 280px ~ 可用宽度的 50%，双击分隔条复位，按应用记忆在 `dsh.review.changesFileWidth`），窄窗口（可用宽度 < 900px）自动退化成上下堆叠。提交信息与「✨ AI 补充」「提交」「提交并推送」固定在**底部**，不随超长文件列表滚走
    - **底部提交区是一块可调高度的 footer**：默认 **8 行**（一行摘要 + 空行 + 5~6 行正文，一条正常的提交信息不用先手动拉高），高度按"行数 × 行高 + 按钮行 + 内边距"算出来，因此跟随「设置 → UI 字号」一起缩放；**顶部有一条横向手柄**（默认几乎不可见，悬停/拖动时显形，光标 `ns-resize`），往上拖变高、往下拖变矮，双击复位到默认高度。高度范围是 `4 行 + 按钮行` ~ `min(视口 55%, Changes 可用高度 65%, 可用高度 − 200px)`——最后一项保证**文件列表 / Diff Preview 始终留下可操作区域**，缩到下限也仍有 4 行正文。用户拖过的高度记在 `dsh.review.commitAreaHeight`；窗口变小只是"这次显示得矮一点"（在渲染期夹取），拉回大窗口原样恢复。输入框自己**不再**有右下角原生 resize（两套高度控制会互相打架），它用 `flex` 填满提交区，因此拖高时变大的只有输入框，按钮行高度固定；按钮下方留 12px，不再贴着窗口底边。
    - **未跟踪文件按数量走两种模式**（阈值 **50**）：**≤ 50** 主面板逐行列出**全部**未跟踪文件（勾选、加入 Git、看差异、变更记录都在，并在有界预算内补精确 `+N`——单文件 ≤ 1 MB、一轮 ≤ 8 MB、二进制只标记不读全文、绝不为一个数字 fork git）；**> 50** 主面板**一行都不列**，只显示"未进行版本管理的文件 6,846 个文件 [浏览]"（旧实现是"列前 50 个 + 还有 6,796 个未显示"，既不是完整列表也不是概要）；精确条数还没数出来时显示"正在统计未跟踪文件…"并按需补一次枚举。
    - **「浏览」是一个惰性目录树弹窗**：目录默认折叠、只请求当前前缀的**直接子节点**，同级节点分页（一页 200 条 + 「继续加载」），因此展开一个躺着 6,835 个文件的目录不会一次 mount 几千行；文件与目录都能勾（目录勾上 = 整棵子树，把目录路径交给 `git add`），部分勾选是 indeterminate，默认全不选，顶部有「全选 / 清空」；加入 Git 后就地刷新已展开的层，主面板同时从 browse 变回 inline（`51 → 加 3 → 48` 这条路径有断言钉住）。
    - **精确枚举是 lazy 的**：只有"Changes 要判定 inline/browse""用户点浏览""写操作之后"才做完整枚举，按 `repositoryRoot` 缓存 20 秒（`repo/src → repo/pages` 直接复用；实测 6,848 条 **115 ms / 1.2 KB**）。
    - **一次加入几千个文件也不会撑爆命令行**：写 NUL 分隔的临时 pathspec 文件走 `git add --pathspec-from-file=… --pathspec-file-nul`（`finally` 清理），老 git 退回"路径数 + 总 argv 字符数"双上限批处理；「全选 → 加入 Git」只发 `{ all: 'untracked' }`，由 host 用它手上那份完整清单去 add——渲染进程永远不持有那几千条路径（实测一次 add 6,835 个路径 4.4 s）。
  - `Log`：**分支树 / 提交图 / 详情** 三栏，单击一条提交只在右侧显示它的详情与改动文件（不再在原位置展开）；两条分栏可拖动调整宽度并记忆，窄窗口可以把分支树或详情收起来；顶部有刷新、计数（`{count} 个提交`，还有更深历史时缀一句「继续滚动加载」）与搜索
    ...
    - **`Log` 与 `Changes` 共用同一个差异视图**（`ReviewDiffViewer`）：头部（状态徽标 + 路径 + `+N −M` + 自动换行 + 关闭）、逐行正文、行号栏、hunk 头与折叠文件头、加载/失败/二进制/截断这几种状态都只有**一处实现**，两个页签的差异长得一样、行为也一样。
    - 差异正文是固定四列：`旧行号 | 新行号 | +/− | 代码`。行号栏固定宽度、右对齐、单独底色 + 右侧描边、不可选中；**默认自动换行**（`white-space: pre-wrap` + `overflow-wrap: anywhere` + `word-break: break-word`，保留缩进/tab 的同时把长行折进可视宽度，长 URL 与 minified 内容也不会撑破容器），此时正文容器**不出现横向滚动条**；头部右侧的「自动换行」按钮可以关掉它——关掉后回到 `white-space: pre`、横向滚动回到容器上。这个偏好是**跨页签共用**的（`Log` 里关掉，`Changes` 也不换行），持久化在 `dsh.review.diffWrap`，默认开启。行高 1.45。
    - **换行不会让行号重复**：一条逻辑行折成三个视觉行时，旧/新行号与 `+` / `−` 标记仍然只显示一次（它们各自是行网格里的一格），整行底色覆盖全部视觉续行。
...
node scripts/test-review-staging.mjs         # 暂存/提交区：三组分组、行内动作、提交框、未跟踪双模式与浏览弹窗、AI 补充交互、提交区高度（8 行默认 / 顶部拖动 / 夹取 / 持久化）
