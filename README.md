...
- **多仓库项目里"当前在看哪个仓库"是整个面板的状态。** 顶部页签右侧出现仓库选择器（仓库名 + 分支 + 改动数，带工作区相对路径），`Changes` 的文件列表与差异、`Log` 的分支树 / 提交图 / 详情、底部提交框**整体**跟着它走；每个仓库仍然各占一格快照、各一套轮询（同一仓库的两个子目录照旧共用一份）。用户没选过时用确定性的默认项（工作区自己所属的仓库 → 列表里的第一个），宿主与客户端用**同一条**规则，因此不会出现"选择器写着 A、面板里是 B"。徽标上的数字是所有仓库**之和**，并缀一句"几个仓库"；**暂存与提交永远只作用于选中的那一个仓库**（客户端为多仓库请求带上 `repository`，宿主只接受"本项目发现过的仓库"，越界一律 400 `repositoryNotAllowed`）。单仓库项目的请求**一个多余参数都不带**，与 1.5.2 完全一致。
