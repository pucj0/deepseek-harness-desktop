...
- **修改提交与安全的历史回退：**提交区可以「修改最后一次提交」（自动读出 HEAD 的信息填回输入框，可只改信息、只加入已暂存内容或两者一起；`--amend` 只替换 HEAD，更早的历史一个字都不动）。如果这个提交**已经推送到上游**，会明确提示"改写已发布的历史"并要求确认一次（不禁止）；之后的推送仍然只能走 `--force-with-lease`，界面永远不会用裸 `--force`。另有「撤销最后一次提交」= `reset --soft HEAD~1`：提交消失、改动留在暂存区、提交信息填回输入框（第一个提交走"去掉 HEAD"的等价路径）。提交图 / Log 里右键任意提交（或点详情栏里的同名按钮）可以「把当前分支重置到这里…」：先给出预览（当前 HEAD、目标提交、影响几个提交），再选 **Soft / Mixed / Hard**——Soft 只动 HEAD、Mixed 顺手取消暂存、Hard 会丢弃已跟踪文件的本地修改（未跟踪文件保留），因此 Hard 必须在写着「Reset Hard」的确认框里点一次才会执行（请求里也带显式确认，宿主会拒绝不带它的 Hard）。重置之后面板会给出一条**撤销入口**，把分支移回原处（Hard 的撤销同样要确认，并说明被丢弃的工作区修改 git 找不回来）。
- **标签（Tags）：**Git 工具条的面板里有独立的**标签分组**（与分支分组并列，搜索框同时过滤它）：每行显示标签名，并标出类型（**附注 / 轻量**）与是否指向 HEAD（「当前版本」）；单击打开标签菜单——签出标记（进入**游离 HEAD**）、从这里新建分支、推送这个标记、复制标记名、删除本地标记、与当前比较。新建标记可写名字与信息（**信息为空即轻量标记**，有信息即附注标记；附注的默认值跟随仓库现状——已有附注标签的项目默认附注）。**推送只推你点的这一个标签**（refspec 是完整的 `refs/tags/<name>`；绝不会 `git push --tags` 把本地所有标记一次推给所有人）。删除只删**本地**标记并需要确认（远端标记管理不在本轮范围内，对话框里写明了这一点）。签出标签后进入游离 HEAD，提示区会给出「从这里新建分支…」。提交图里右键任意提交也有「在此创建标记…」。
- **比较两个修订（Compare）：**提交右键菜单里的「与当前比较」把那条提交与 HEAD 比；「选择用于比较」+「与所选提交比较」可以比任意两条提交（A ↔ B）；gitbar 里分支/标签菜单的「与当前比较」通过跨插件入口打开**同一个**比较视图。比较视图给出两端是谁、各自有几个提交是对方没有的（`git rev-list --left-right --count`）、以及改动的文件清单（含改名），点文件就在下面用**同一个差异查看器**看这两个修订之间的差异——没有第二套差异引擎。
- **文件历史（File History）：**改动列表里每一行的「查看文件历史」用 `git log --follow` 列出该文件的提交（日期 / 作者 / 提交信息 / SHA，**改名之前的历史也追得到**）；点一条历史就在右侧看**那个提交里对这个文件的改动**（`/commit-file` + 同一个差异查看器，与提交图里点文件走同一条路）。
...
- **提交右键菜单：**提交图 / Log 里右键任意提交可以复制完整的提交 SHA、与当前比较、选择用于比较、在此创建分支、在此创建标记、摘取（cherry-pick）、还原（revert），以及**单独成组**的「把当前分支重置到这里…」（会改写当前分支的动作与只读动作在视觉上分开，并带一句说明）。

打包需在对应平台执行。`npm run test:locale` 验证外壳语言跟随 Harness：语言归一化、设置文档解析与监听、菜单模板（中英对照 + 命令不变）、运行中切换（真实应用实例，不重启）、切换工作区不重置语言，以及标题栏按钮与文档 `lang` 的同步。`npm run test:git` 依次跑 Git 工作流的全部回归测试：**修改提交与重置**、**标签与修订比较**（轻量/附注标签、签出进入游离 HEAD、从标签建分支、删除本地标签、只推单个标签、提交 ↔ HEAD / A ↔ B / 分支 ↔ 当前 的比较、改名跟踪的文件历史、多仓库隔离）、冲突（合并 / 变基 / 摘取 / 还原，含**多轮变基**）、**储藏全流程**、端到端发布流程、分支条交互与源面板（含标签分组与提交菜单）、改动审查的冲突界面、并排/统一差异视图、储藏面板、提交区（amend / 撤销 / 提交并推送）与暂存区（含文件历史）。`scripts/` 还包含 Git 分支、仓库发现、改动审查、暂存与提交、插件同步、运行时准备及 Electron/CDP 冒烟测试；涉及真实窗口的测试需要图形环境。
