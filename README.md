...
- 顶部是 `Changes / Log` 两个页签（IDEA 的 Git 工具窗同款）：
  - `Changes`：**已暂存 / 更改 / 未跟踪** 三组（同一份快照过滤得出，分组数量与列表恒等），分成**左右两栏**——左栏勾选"我要提交哪些文件"（每行可暂存 / 取消暂存 / 还原 / 查看该文件的变更记录），右栏是"这个文件具体改了什么"的差异预览；两栏之间可拖动（默认左栏占 **34%**，拖动范围 280px ~ 可用宽度的 50%，双击分隔条复位，按应用记忆在 `dsh.review.changesFileWidth`），窄窗口（可用宽度 < 900px）自动退化成上下堆叠。提交信息（4 行高、可纵向拖大）与「✨ AI 补充」「提交」「提交并推送」固定在**底部**，不随超长文件列表滚走
