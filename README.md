...
打包需在对应平台执行。`npm run test:locale` 验证外壳语言跟随 Harness：语言归一化、设置文档解析与监听、菜单模板（中英对照 + 命令不变）、运行中切换（真实应用实例，不重启）、切换工作区不重置语言，以及标题栏按钮与文档 `lang` 的同步。`npm run test:git` 依次跑 Git 工作流的全部回归测试：冲突（合并 / 变基 / 摘取 / 还原，含**多轮变基**，全部在真实临时仓库上）、**储藏全流程**（快速/带消息/含未跟踪的储藏、列表、查看、应用、弹出、删除、储藏并切换、储藏冲突、多仓库隔离）、端到端发布流程、分支条交互与源面板（含储藏的快捷入口与冲突卡片）、改动审查的冲突界面（合并编辑器的逐块导航与"只算不写"的 Result 预览）、并排/统一差异视图、储藏面板与暂存区。`scripts/` 还包含 Git 分支、仓库发现、改动审查、暂存与提交、插件同步、运行时准备及 Electron/CDP 冒烟测试；涉及真实窗口的测试需要图形环境。
